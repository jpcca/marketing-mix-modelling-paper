...
\begin{abstract}
Marketing Mix Models (MMM) are widely used for measuring advertising effectiveness, yet standard implementations assume homogeneous consumer response to marketing spend. This paper proposes a Bayesian mixture of Hill saturation functions to capture heterogeneous response patterns across latent consumer segments. We implement the model using NumPyro with automatic prior scaling and develop post-hoc relabeling diagnostics to address label switching in mixture posteriors. Comprehensive experiments on synthetic data (80 configurations) and real marketing data from 10 organizations (90 experiments) demonstrate that: (1) mixture models achieve substantial ELPD improvements of +21 to +1329 over single-curve baselines when heterogeneity is present; (2) the $K=2$ mixture with relabeled R-hat diagnostics achieves 43\% convergence on real data compared to 7\% with standard diagnostics; and (3) the $K=3$ mixture provides the best predictive performance but faces fundamental identifiability challenges on real data. We provide practical recommendations for practitioners choosing between model complexity and convergence reliability.
\end{abstract}
...
A key methodological contribution is our treatment of label switching in mixture posteriors. Standard convergence diagnostics (R-hat) are unreliable for mixture models because component labels may exchange across MCMC chains. We develop post-hoc relabeling based on component ordering and demonstrate that this approach raises the convergence rate for $K=2$ mixtures from 7\% to 43\% on real data.
...
\paragraph{Model comparison.} We compare four specifications: (1)~\textbf{Single Hill} with one global saturation curve, (2)~\textbf{Mixture $K=2$} with two components, (3)~\textbf{Mixture $K=3$} with three components, and (4)~\textbf{Sparse $K=5$} with five components and Dirichlet concentration 0.5 for automatic pruning. Inference uses NUTS with 1,000 warmup and 2,000 sampling iterations across 4 chains.
...
\begin{tabular}{lcccc}
\toprule
True $K$ & Single Hill & Mixture $K=2$ & Mixture $K=3$ & Sparse $K=5$ \\
\midrule
$K=1$ & 100 & 60 & 100 & 60 \\
$K=2$ & 100 & 20 & \textbf{100} & 60 \\
$K=3$ & 100 & 0 & 40 & 40 \\
$K=5$ & 100 & 40 & 80 & 60 \\
\bottomrule
\end{tabular}
...
Key findings: (1) Single Hill always converges but cannot capture mixture structure. (2) Mixture $K=3$ is the most robust mixture model with 40--100\% convergence across DGPs. (3) Mixture $K=2$ has surprisingly poor convergence, often worse than $K=3$.
...
$K=2$ & Mixture $K=3$ & $-406.4 \pm 8.7$ & $5.2 \pm 0.9$ & $+3.4$ \\
\addlinespace
$K=3$ & Single Hill & $-520.7 \pm 7.4$ & $8.3 \pm 1.0$ & --- \\
$K=3$ & Mixture $K=3$ & $-488.1 \pm 9.5$ & $8.3 \pm 0.9$ & $\mathbf{+32.7}$ \\
$K=3$ & Sparse $K=5$ & $-488.7 \pm 8.9$ & $8.3 \pm 1.0$ & $\mathbf{+31.6}$ \\
\addlinespace
$K=5$ & Single Hill & $-509.5 \pm 4.7$ & $7.8 \pm 0.9$ & --- \\
$K=5$ & Mixture $K=3$ & $-498.9 \pm 5.4$ & $7.7 \pm 0.9$ & $+10.6$ \\
\bottomrule
\end{tabular}
\end{table}

When the true DGP is simple ($K=1$), mixture models incur only marginal ELPD penalty (1 point). For heterogeneous data ($K \geq 3$), mixture models achieve significant ELPD improvements of +31 to +33 points.
...
Our experiments reveal a fundamental trade-off between predictive performance and convergence reliability in Hill mixture models.

\paragraph{The $K=2$ vs $K=3$ dilemma.} Mixture $K=3$ consistently achieves the best ELPD on both synthetic and real data, winning on 9/10 organizations. However, it never converges on real data (0\% by any diagnostic). Mixture $K=2$ has inferior ELPD but achieves 43\% convergence with relabeled diagnostics. For practitioners requiring reliable uncertainty quantification, $K=2$ with relabeling is the recommended choice.
...
We presented a Bayesian mixture of Hill saturation functions for capturing heterogeneous consumer response in Marketing Mix Modeling. The approach achieves ELPD improvements of +21 to +1,329 compared to single-curve baselines when heterogeneity is present, while maintaining competitive performance on homogeneous data.

Our key methodological contribution is the post-hoc relabeling approach for mixture convergence diagnostics. This simple technique raises the practical convergence rate for $K=2$ mixtures from 7\% to 43\% on real data, making the mixture approach viable for production use.

The tension between $K=3$'s superior predictive performance and $K=2$'s convergence reliability reflects a fundamental challenge in mixture modeling: more flexible models capture heterogeneity better but are harder to fit reliably. Our recommendation of $K=2$ with relabeling provides a practical middle ground for practitioners.
